...
# 截断染色函数，将图像的两边染色，第二个参数为左边染色1/lw，第三个参数为右边染色1/rw,第四个参数为染色的灰度值
def dye(image, lw=5, rw=5, value = 255):
    h, w = image.shape
    mask = np.ones([h,w],np.uint8)
    mask *=value
    mask[0:h,int(w/lw): int(w-w/rw)] = 0
...
def process(path):
    ...
    def hough(src, dst):
        ...
        lines = cv.HoughLines(res, 1, np.pi/180, 40)
        background = src.copy()
        for line in lines:
...
sub_path2 = "/1668600.bmp"
sub_path3 = "/1669200.bmp"
sub_path4 = "/1462000.bmp"

path_zfz = "D:/study/opencv/detection/1460000.bmp"

show("img", process(path_wsj+sub_path4))
